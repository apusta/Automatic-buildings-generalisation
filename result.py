...
class Results:
    ...
    def save(self):
        if self.eliminated_buildings:
            with fiona.open('./10_luz/eliminated.shp', 'w', 'ESRI Shapefile', self._SCHEMA, crs=from_epsg(2177)) as file:
        ...
        if self.rectangelized_buildings:
            with fiona.open('./10_luz/generalised_to_rectangle.shp', 'w', 'ESRI Shapefile', self._RECTANGLE_SCHEMA,
        ...
        if self.for_manual_revision_buildings:
            with fiona.open('./10_luz/notGeneralised.shp', 'w', 'ESRI Shapefile', self._SCHEMA,
                            crs=from_epsg(2177)) as file:
        ...
        if self.signatured_buildings:
            with fiona.open('./10_luz/generalised_to_signature.shp', 'w', 'ESRI Shapefile', self._SIGNATURE_SCHEMA,
                            crs=from_epsg(2177)) as file:
        ...
        if self.unioned_groups:
            with fiona.open('./10_luz/union.shp', 'w', 'ESRI Shapefile', self._GROUP_SCHEMA, crs=from_epsg(2177)) as file:
        ...
        if self.typificated_groups:
            with fiona.open('./10_luz/typificated.shp', 'w', 'ESRI Shapefile', self._GROUP_SCHEMA, crs=from_epsg(2177)) as file:
